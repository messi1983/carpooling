...
export class CarpoolingUtils {
  ...
  public static addToList(
    carpooling: CarpoolingViewModel,
    carpoolings: CarpoolingViewModel[]
  ): void {
  ...
  public static existAcceptionAutoSurSelectionsAller(
    selections: CarpoolingViewModel[]
  ): boolean {
    return (
      selections.filter(
        (cp: CarpoolingViewModel) =>
          cp.aller && cp.aller.checked && cp.aller.acceptationAuto
      ).length !== 0
    );
  }

  public static existAcceptionAutoSurSelectionsRetour(
    selections: CarpoolingViewModel[]
  ): boolean {
    return (
      selections.filter(
        (cp: CarpoolingViewModel) =>
          cp.retour && cp.retour.checked && cp.retour.acceptationAuto
      ).length !== 0
    );
  }

  public static countSelections(
    selections: CarpoolingViewModel[],
    isAller: boolean
  ): number {
    if (isAller) {
  ...
  public static filterSelectionsAller(
    selections: CarpoolingViewModel[]
  ): CarpoolingViewModel[] {
    return selections.filter(
      (cp: CarpoolingViewModel) => cp.aller && cp.aller.checked
    );
  }

  public static countSelectionsAller(
    selections: CarpoolingViewModel[]
  ): number {
    return CarpoolingUtils.filterSelectionsAller(selections).length;
  }

  public static filterSelectionsRetour(
    selections: CarpoolingViewModel[]
  ): CarpoolingViewModel[] {
    return selections.filter(
      (cp: CarpoolingViewModel) => cp.retour && cp.retour.checked
    );
  }

  public static countSelectionsRetour(
    selections: CarpoolingViewModel[]
  ): number {
    return CarpoolingUtils.filterSelectionsRetour(selections).length;
  }

  public static removeFromList(
    carpooling: CarpoolingViewModel,
    carpoolings: CarpoolingViewModel[],
    isAller: boolean
  ): void {
