...
export class CarpoolingsListComponent extends AbstractStepperComponent
  ...
  ngOnInit() {
    this._carpoolingService
      .getCarpoolingsList(this.trajetAller, "18-12-2017", 4, true, true)
      .subscribe(carpoolings => {
        this.carpoolingsAller.push.apply(
          this.carpoolingsAller,
          this._carpoolingViewModelService.creerListeCarpoolingsAller(
            carpoolings
          )
        );
      });
  ...
  goToStepRetours(): void {
    if (this.carpoolingsRetour.length === 0) {
      this._carpoolingService
        .getCarpoolingsList(this.trajetRetour, "18-12-2017", 4, true, true)
        .subscribe(carpoolings => {
          this.carpoolingsRetour.push.apply(
            this.carpoolingsRetour,
            this._carpoolingViewModelService.creerListeCarpoolingsRetour(
              carpoolings,
              this.carpoolingsAller
            )
          );
